...
const roles: Role[] = ["owner", "admin", "api_admin", "api_operator", "viewer"];
const port = Number(process.env.PORT ?? 10710);
...
const service = new APIService(store);
...
const server = createServer(async (req, res) => {
  try {
    ...
    if (req.method === "GET" && url.pathname === "/health") { sendJson(res, 200, { service: "APIOS", status: "ok", message: service.getRoutesSummary() }); return; }
    if (req.method === "GET" && url.pathname === "/docs") { sendJson(res, 200, docs()); return; }
    if (parts[0] !== "apios") notFound("Route not found");
...
server.listen(port, () => { console.log("APIOS listening on http://localhost:" + port); });
