...
const { AdminService } = require("../dist/services/adminos.service.js");
const { createSeedState } = require("../dist/seed-state.js");

function createService() {
  const file = join("/tmp", "adminos-test-" + Date.now() + "-" + Math.random().toString(36).slice(2) + ".json");
  const store = new DataStore(file);
  store.reset(createSeedState("demo-tenant"));
  const service = new AdminService(store, new EventBus(store));
  const actor = { tenantId: "demo-tenant", userId: "test-user", role: "adminos_admin" };
...
test("AdminOS overview uses seed data", () => {
...
test("AdminOS creates items, workflows, and runs", () => {
  const { service, actor, file } = createService();
  try {
...
test("AdminOS creates policies and audits changes", () => {
  const { service, actor, file } = createService();
  try {
