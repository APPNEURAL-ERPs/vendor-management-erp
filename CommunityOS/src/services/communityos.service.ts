import { CommunityItem, CommunityOverview, CommunityPolicy, CommunityWorkflow, CommunityWorkRun, EntityStatus, Priority, RequestActor, WorkStatus } from "../core/domain";
...
export class CommunityService {
  ...
  overview(actor: RequestActor): CommunityOverview {
  ...
  listItems(actor: RequestActor, query?: URLSearchParams): CommunityItem[] {
  ...
  createItem(actor: RequestActor, input: Partial<CommunityItem>): CommunityItem {
    const state = this.store.getState();
    ...
    const item: CommunityItem = {
      ...
      category: ensureString(input.category ?? "community", "category"),
      ownerTeam: ensureString(input.ownerTeam ?? "Community", "ownerTeam"),
    ...
    this.store.save();
    this.store.audit(actor, "communityos.item.create", "item", item.id, undefined, item);
    this.events.publish(actor, "communityos.item.created", { itemKey: item.key });
    return clone(item);
  }

  updateItem(actor: RequestActor, key: string, input: Partial<CommunityItem>): CommunityItem {
    ...
    this.store.save();
    this.store.audit(actor, "communityos.item.update", "item", item.id, before, item);
    this.events.publish(actor, "communityos.item.updated", { itemKey: item.key });
    return clone(item);
  }

  archiveItem(actor: RequestActor, key: string): CommunityItem {
    return this.updateItem(actor, key, { status: "archived" });
  }

  listWorkflows(actor: RequestActor): CommunityWorkflow[] {
    return clone(this.store.getState().workflows.filter((item) => item.tenantId === actor.tenantId && item.status !== "archived"));
  }

  createWorkflow(actor: RequestActor, input: Partial<CommunityWorkflow>): CommunityWorkflow {
    const state = this.store.getState();
    const key = normalizeKey(input.key ?? input.name);
    this.ensureUnique(state.workflows, actor.tenantId, key, "Workflow");
    const now = nowIso();
    const workflow: CommunityWorkflow = {
      id: newId("flow"),
      tenantId: actor.tenantId,
      createdAt: now,
      updatedAt: now,
      ...
      description: optionalString(input.description),
      category: ensureString(input.category ?? "forum", "category"),
      status: this.ensureStatus(input.status ?? "active"),
    ...
    this.store.save();
    this.store.audit(actor, "communityos.workflow.create", "workflow", workflow.id, undefined, workflow);
    this.events.publish(actor, "communityos.workflow.created", { workflowKey: workflow.key });
    return clone(workflow);
  }

  startWorkflow(actor: RequestActor, key: string, input: Record<string, unknown> = {}): CommunityWorkRun {
    const workflow = this.findWorkflow(actor, key);
    if (workflow.status !== "active") badRequest("Only active workflows can be started");
    const now = nowIso();
    const run: CommunityWorkRun = {
      id: newId("run"),
      tenantId: actor.tenantId,
      createdAt: now,
      updatedAt: now,
    ...
    this.store.save();
    this.store.audit(actor, "communityos.run.create", "work_run", run.id, undefined, run);
    this.events.publish(actor, "communityos.workflow.started", { runId: run.id, workflowKey: run.workflowKey });
    return clone(run);
  }

  updateRun(actor: RequestActor, id: string, input: Partial<CommunityWorkRun>): CommunityWorkRun {
    ...
    this.store.save();
    this.store.audit(actor, "communityos.run.update", "work_run", run.id, before, run);
    this.events.publish(actor, "communityos.run.updated", { runId: run.id, status: run.status });
    return clone(run);
  }

  listRuns(actor: RequestActor): CommunityWorkRun[] {
    return clone(this.store.getState().workRuns.filter((item) => item.tenantId === actor.tenantId));
  }

  createPolicy(actor: RequestActor, input: Partial<CommunityPolicy>): CommunityPolicy {
    const state = this.store.getState();
    const key = normalizeKey(input.key ?? input.title);
    ...
    this.store.save();
    this.store.audit(actor, "communityos.policy.create", "policy", policy.id, undefined, policy);
    this.events.publish(actor, "communityos.policy.created", { policyKey: policy.key });
    return clone(policy);
  }

  listPolicies(actor: RequestActor): CommunityPolicy[] {
  ...
  private findItem(actor: RequestActor, key: string): CommunityItem {
  ...
  private findWorkflow(actor: RequestActor, key: string): CommunityWorkflow {
  ...
  private findRun(actor: RequestActor, id: string): CommunityWorkRun {
