...
const { CommunityService } = require("../dist/services/communityos.service.js");
const { createSeedState } = require("../dist/seed-state.js");

function createService() {
  const file = join("/tmp", "communityos-test-" + Date.now() + "-" + Math.random().toString(36).slice(2) + ".json");
  const store = new DataStore(file);
  store.reset(createSeedState("demo-tenant"));
  const service = new CommunityService(store, new EventBus(store));
  const actor = { tenantId: "demo-tenant", userId: "test-user", role: "communityos_admin" };
...
test("CommunityOS overview uses seed data", () => {
...
test("CommunityOS creates items, workflows, and runs", () => {
  const { service, actor, file } = createService();
  try {
...
test("CommunityOS creates policies and audits changes", () => {
  const { service, actor, file } = createService();
  try {
