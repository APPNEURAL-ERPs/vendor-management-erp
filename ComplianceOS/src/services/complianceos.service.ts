import { ComplianceItem, ComplianceOverview, CompliancePolicy, ComplianceWorkflow, ComplianceWorkRun, EntityStatus, Priority, RequestActor, WorkStatus } from "../core/domain";
...
export class ComplianceService {
  ...
  listItems(actor: RequestActor, query?: URLSearchParams): ComplianceItem[] {
  ...
  createItem(actor: RequestActor, input: Partial<ComplianceItem>): ComplianceItem {
    const state = this.store.getState();
    ...
    const item: ComplianceItem = {
      ...
      category: ensureString(input.category ?? "policy", "category"),
      ownerTeam: ensureString(input.ownerTeam ?? "Compliance", "ownerTeam"),
    ...
    this.store.save();
    this.store.audit(actor, "complianceos.item.create", "item", item.id, undefined, item);
    this.events.publish(actor, "complianceos.item.created", { itemKey: item.key });
    return clone(item);
  }

  updateItem(actor: RequestActor, key: string, input: Partial<ComplianceItem>): ComplianceItem {
    ...
    this.store.save();
    this.store.audit(actor, "complianceos.item.update", "item", item.id, before, item);
    this.events.publish(actor, "complianceos.item.updated", { itemKey: item.key });
    return clone(item);
  }

  archiveItem(actor: RequestActor, key: string): ComplianceItem {
    return this.updateItem(actor, key, { status: "archived" });
  }

  listWorkflows(actor: RequestActor): ComplianceWorkflow[] {
    return clone(this.store.getState().workflows.filter((item) => item.tenantId === actor.tenantId && item.status !== "archived"));
  }

  createWorkflow(actor: RequestActor, input: Partial<ComplianceWorkflow>): ComplianceWorkflow {
    const state = this.store.getState();
    ...
    const workflow: ComplianceWorkflow = {
      ...
      description: optionalString(input.description),
      category: ensureString(input.category ?? "audit", "category"),
      status: this.ensureStatus(input.status ?? "active"),
    ...
    this.store.save();
    this.store.audit(actor, "complianceos.workflow.create", "workflow", workflow.id, undefined, workflow);
    this.events.publish(actor, "complianceos.workflow.created", { workflowKey: workflow.key });
    return clone(workflow);
  }

  startWorkflow(actor: RequestActor, key: string, input: Record<string, unknown> = {}): ComplianceWorkRun {
    const workflow = this.findWorkflow(actor, key);
    if (workflow.status !== "active") badRequest("Only active workflows can be started");
    const now = nowIso();
    const run: ComplianceWorkRun = {
      id: newId("run"),
      tenantId: actor.tenantId,
      createdAt: now,
      updatedAt: now,
    ...
    this.store.save();
    this.store.audit(actor, "complianceos.run.create", "work_run", run.id, undefined, run);
    this.events.publish(actor, "complianceos.workflow.started", { runId: run.id, workflowKey: run.workflowKey });
    return clone(run);
  }

  updateRun(actor: RequestActor, id: string, input: Partial<ComplianceWorkRun>): ComplianceWorkRun {
    ...
    this.store.save();
    this.store.audit(actor, "complianceos.run.update", "work_run", run.id, before, run);
    this.events.publish(actor, "complianceos.run.updated", { runId: run.id, status: run.status });
    return clone(run);
  }

  listRuns(actor: RequestActor): ComplianceWorkRun[] {
    return clone(this.store.getState().workRuns.filter((item) => item.tenantId === actor.tenantId));
  }

  createPolicy(actor: RequestActor, input: Partial<CompliancePolicy>): CompliancePolicy {
    const state = this.store.getState();
    const key = normalizeKey(input.key ?? input.title);
    ...
    this.store.save();
    this.store.audit(actor, "complianceos.policy.create", "policy", policy.id, undefined, policy);
    this.events.publish(actor, "complianceos.policy.created", { policyKey: policy.key });
    return clone(policy);
  }

  listPolicies(actor: RequestActor): CompliancePolicy[] {
  ...
  private findItem(actor: RequestActor, key: string): ComplianceItem {
  ...
  private findWorkflow(actor: RequestActor, key: string): ComplianceWorkflow {
  ...
  private findRun(actor: RequestActor, id: string): ComplianceWorkRun {
