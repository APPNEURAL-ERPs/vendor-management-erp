...
const { ComplianceService } = require("../dist/services/complianceos.service.js");
const { createSeedState } = require("../dist/seed-state.js");

function createService() {
  const file = join("/tmp", "complianceos-test-" + Date.now() + "-" + Math.random().toString(36).slice(2) + ".json");
  const store = new DataStore(file);
  store.reset(createSeedState("demo-tenant"));
  const service = new ComplianceService(store, new EventBus(store));
  const actor = { tenantId: "demo-tenant", userId: "test-user", role: "complianceos_admin" };
...
test("ComplianceOS overview uses seed data", () => {
...
test("ComplianceOS creates items, workflows, and runs", () => {
  const { service, actor, file } = createService();
  try {
...
test("ComplianceOS creates policies and audits changes", () => {
  const { service, actor, file } = createService();
  try {
