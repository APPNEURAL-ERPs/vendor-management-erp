import { ContentItem, ContentOverview, ContentPolicy, ContentWorkflow, ContentWorkRun, EntityStatus, Priority, RequestActor, WorkStatus } from "../core/domain";
...
export class ContentService {
  ...
  listItems(actor: RequestActor, query?: URLSearchParams): ContentItem[] {
  ...
  createItem(actor: RequestActor, input: Partial<ContentItem>): ContentItem {
    const state = this.store.getState();
    ...
    const item: ContentItem = {
      ...
      category: ensureString(input.category ?? "blog", "category"),
      ownerTeam: ensureString(input.ownerTeam ?? "Content", "ownerTeam"),
    ...
    this.store.save();
    this.store.audit(actor, "contentos.item.create", "item", item.id, undefined, item);
    this.events.publish(actor, "contentos.item.created", { itemKey: item.key });
    return clone(item);
  }

  updateItem(actor: RequestActor, key: string, input: Partial<ContentItem>): ContentItem {
    ...
    this.store.save();
    this.store.audit(actor, "contentos.item.update", "item", item.id, before, item);
    this.events.publish(actor, "contentos.item.updated", { itemKey: item.key });
    return clone(item);
  }

  archiveItem(actor: RequestActor, key: string): ContentItem {
    return this.updateItem(actor, key, { status: "archived" });
  }

  listWorkflows(actor: RequestActor): ContentWorkflow[] {
    return clone(this.store.getState().workflows.filter((item) => item.tenantId === actor.tenantId && item.status !== "archived"));
  }

  createWorkflow(actor: RequestActor, input: Partial<ContentWorkflow>): ContentWorkflow {
    const state = this.store.getState();
    ...
    const workflow: ContentWorkflow = {
      ...
      description: optionalString(input.description),
      category: ensureString(input.category ?? "social", "category"),
      status: this.ensureStatus(input.status ?? "active"),
    ...
    this.store.save();
    this.store.audit(actor, "contentos.workflow.create", "workflow", workflow.id, undefined, workflow);
    this.events.publish(actor, "contentos.workflow.created", { workflowKey: workflow.key });
    return clone(workflow);
  }

  startWorkflow(actor: RequestActor, key: string, input: Record<string, unknown> = {}): ContentWorkRun {
    const workflow = this.findWorkflow(actor, key);
    if (workflow.status !== "active") badRequest("Only active workflows can be started");
    const now = nowIso();
    const run: ContentWorkRun = {
      id: newId("run"),
      tenantId: actor.tenantId,
      createdAt: now,
      updatedAt: now,
    ...
    this.store.save();
    this.store.audit(actor, "contentos.run.create", "work_run", run.id, undefined, run);
    this.events.publish(actor, "contentos.workflow.started", { runId: run.id, workflowKey: run.workflowKey });
    return clone(run);
  }

  updateRun(actor: RequestActor, id: string, input: Partial<ContentWorkRun>): ContentWorkRun {
    ...
    this.store.save();
    this.store.audit(actor, "contentos.run.update", "work_run", run.id, before, run);
    this.events.publish(actor, "contentos.run.updated", { runId: run.id, status: run.status });
    return clone(run);
  }

  listRuns(actor: RequestActor): ContentWorkRun[] {
    return clone(this.store.getState().workRuns.filter((item) => item.tenantId === actor.tenantId));
  }

  createPolicy(actor: RequestActor, input: Partial<ContentPolicy>): ContentPolicy {
    const state = this.store.getState();
    const key = normalizeKey(input.key ?? input.title);
    ...
    this.store.save();
    this.store.audit(actor, "contentos.policy.create", "policy", policy.id, undefined, policy);
    this.events.publish(actor, "contentos.policy.created", { policyKey: policy.key });
    return clone(policy);
  }

  listPolicies(actor: RequestActor): ContentPolicy[] {
  ...
  private findItem(actor: RequestActor, key: string): ContentItem {
  ...
  private findWorkflow(actor: RequestActor, key: string): ContentWorkflow {
  ...
  private findRun(actor: RequestActor, id: string): ContentWorkRun {
