...
const { ContentService } = require("../dist/services/contentos.service.js");
const { createSeedState } = require("../dist/seed-state.js");

function createService() {
  const file = join("/tmp", "contentos-test-" + Date.now() + "-" + Math.random().toString(36).slice(2) + ".json");
  const store = new DataStore(file);
  store.reset(createSeedState("demo-tenant"));
  const service = new ContentService(store, new EventBus(store));
  const actor = { tenantId: "demo-tenant", userId: "test-user", role: "contentos_admin" };
...
test("ContentOS overview uses seed data", () => {
...
test("ContentOS creates items, workflows, and runs", () => {
  const { service, actor, file } = createService();
  try {
...
test("ContentOS creates policies and audits changes", () => {
  const { service, actor, file } = createService();
  try {
