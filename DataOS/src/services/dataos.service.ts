import { DataItem, DataOverview, DataPolicy, DataWorkflow, DataWorkRun, EntityStatus, Priority, RequestActor, WorkStatus } from "../core/domain";
...
export class DataService {
  ...
  overview(actor: RequestActor): DataOverview {
  ...
  listItems(actor: RequestActor, query?: URLSearchParams): DataItem[] {
  ...
  createItem(actor: RequestActor, input: Partial<DataItem>): DataItem {
    const state = this.store.getState();
    ...
    const item: DataItem = {
      ...
      category: ensureString(input.category ?? "warehouse", "category"),
      ownerTeam: ensureString(input.ownerTeam ?? "Data", "ownerTeam"),
    ...
    this.store.save();
    this.store.audit(actor, "dataos.item.create", "item", item.id, undefined, item);
    this.events.publish(actor, "dataos.item.created", { itemKey: item.key });
    return clone(item);
  }

  updateItem(actor: RequestActor, key: string, input: Partial<DataItem>): DataItem {
    ...
    this.store.save();
    this.store.audit(actor, "dataos.item.update", "item", item.id, before, item);
    this.events.publish(actor, "dataos.item.updated", { itemKey: item.key });
    return clone(item);
  }

  archiveItem(actor: RequestActor, key: string): DataItem {
    return this.updateItem(actor, key, { status: "archived" });
  }

  listWorkflows(actor: RequestActor): DataWorkflow[] {
    return clone(this.store.getState().workflows.filter((item) => item.tenantId === actor.tenantId && item.status !== "archived"));
  }

  createWorkflow(actor: RequestActor, input: Partial<DataWorkflow>): DataWorkflow {
    const state = this.store.getState();
    ...
    const workflow: DataWorkflow = {
      ...
      description: optionalString(input.description),
      category: ensureString(input.category ?? "lake", "category"),
      status: this.ensureStatus(input.status ?? "active"),
    ...
    this.store.save();
    this.store.audit(actor, "dataos.workflow.create", "workflow", workflow.id, undefined, workflow);
    this.events.publish(actor, "dataos.workflow.created", { workflowKey: workflow.key });
    return clone(workflow);
  }

  startWorkflow(actor: RequestActor, key: string, input: Record<string, unknown> = {}): DataWorkRun {
    const workflow = this.findWorkflow(actor, key);
    if (workflow.status !== "active") badRequest("Only active workflows can be started");
    const now = nowIso();
    const run: DataWorkRun = {
      id: newId("run"),
      tenantId: actor.tenantId,
      createdAt: now,
      updatedAt: now,
    ...
    this.store.save();
    this.store.audit(actor, "dataos.run.create", "work_run", run.id, undefined, run);
    this.events.publish(actor, "dataos.workflow.started", { runId: run.id, workflowKey: run.workflowKey });
    return clone(run);
  }

  updateRun(actor: RequestActor, id: string, input: Partial<DataWorkRun>): DataWorkRun {
    ...
    this.store.save();
    this.store.audit(actor, "dataos.run.update", "work_run", run.id, before, run);
    this.events.publish(actor, "dataos.run.updated", { runId: run.id, status: run.status });
    return clone(run);
  }

  listRuns(actor: RequestActor): DataWorkRun[] {
    return clone(this.store.getState().workRuns.filter((item) => item.tenantId === actor.tenantId));
  }

  createPolicy(actor: RequestActor, input: Partial<DataPolicy>): DataPolicy {
    const state = this.store.getState();
    const key = normalizeKey(input.key ?? input.title);
    ...
    this.store.save();
    this.store.audit(actor, "dataos.policy.create", "policy", policy.id, undefined, policy);
    this.events.publish(actor, "dataos.policy.created", { policyKey: policy.key });
    return clone(policy);
  }

  listPolicies(actor: RequestActor): DataPolicy[] {
  ...
  private findItem(actor: RequestActor, key: string): DataItem {
  ...
  private findWorkflow(actor: RequestActor, key: string): DataWorkflow {
  ...
  private findRun(actor: RequestActor, id: string): DataWorkRun {
