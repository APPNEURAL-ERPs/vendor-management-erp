...
const { DataService } = require("../dist/services/dataos.service.js");
const { createSeedState } = require("../dist/seed-state.js");

function createService() {
  const file = join("/tmp", "dataos-test-" + Date.now() + "-" + Math.random().toString(36).slice(2) + ".json");
  const store = new DataStore(file);
  store.reset(createSeedState("demo-tenant"));
  const service = new DataService(store, new EventBus(store));
  const actor = { tenantId: "demo-tenant", userId: "test-user", role: "dataos_admin" };
...
test("DataOS overview uses seed data", () => {
...
test("DataOS creates items, workflows, and runs", () => {
  const { service, actor, file } = createService();
  try {
...
test("DataOS creates policies and audits changes", () => {
  const { service, actor, file } = createService();
  try {
