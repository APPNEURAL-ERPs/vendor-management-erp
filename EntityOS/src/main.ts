...
const roles: Role[] = ["owner", "admin", "entity_admin", "entity_operator", "viewer"];
const port = Number(process.env.PORT ?? 10712);
...
const server = createServer(async (req, res) => {
  try {
    ...
    if (req.method === "GET" && url.pathname === "/health") { sendJson(res, 200, { service: "EntityOS", status: "ok", message: service.getRoutesSummary() }); return; }
    if (req.method === "GET" && url.pathname === "/docs") { sendJson(res, 200, docs()); return; }
    if (parts[0] !== "entityos") notFound("Route not found");
...
server.listen(port, () => { console.log("EntityOS listening on http://localhost:" + port); });
