...
export function matchesFilter(data: Record<string, unknown>, filter: Record<string, unknown>): boolean {
  for (const [key, value] of Object.entries(filter)) {
    const dataValue = getPathValue(data, key);
    if (value === undefined) {
...
export function calculateBackoffDelay(attempt: number, initialDelayMs: number, maxDelayMs: number, multiplier: number): number {
  const delay = Math.min(initialDelayMs * Math.pow(multiplier, attempt), maxDelayMs);
  return delay;
}
