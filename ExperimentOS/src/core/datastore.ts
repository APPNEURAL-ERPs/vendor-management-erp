...
import { ExperimentosState, AuditLog, RequestActor } from "./domain";
import { newId, nowIso } from "./id";
import { clone } from "./utils";

export function emptyState(): ExperimentosState {
  return {
    hypotheses: [],
    experiments: [],
    variants: [],
...
export class DataStore {
  private state: ExperimentosState;
  ...
  getState(): ExperimentosState {
    return this.state;
  }

  snapshot(): ExperimentosState {
    return clone(this.state);
  }

  replaceState(nextState: ExperimentosState): void {
