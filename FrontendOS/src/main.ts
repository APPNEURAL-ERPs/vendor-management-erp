...
const roles: Role[] = ["owner", "admin", "frontend_admin", "frontend_operator", "viewer"];
const port = Number(process.env.PORT ?? 10713);
const dbFile = process.env.FRONTENDOS_DB_FILE ?? "data/frontendos.db.json";
...
const service = new FrontendService(store);
...
const server = createServer(async (req, res) => {
  try {
    ...
    if (req.method === "GET" && url.pathname === "/health") { sendJson(res, 200, { service: "FrontendOS", status: "ok", message: service.getRoutesSummary() }); return; }
    if (req.method === "GET" && url.pathname === "/docs") { sendJson(res, 200, docs()); return; }
    if (parts[0] !== "frontendos") notFound("Route not found");
...
server.listen(port, () => { console.log("FrontendOS listening on http://localhost:" + port); });
