...
const { GovernanceService } = require("../dist/services/governanceos.service.js");
const { createSeedState } = require("../dist/seed-state.js");

function createService() {
  const file = join("/tmp", "governanceos-test-" + Date.now() + "-" + Math.random().toString(36).slice(2) + ".json");
  const store = new DataStore(file);
  store.reset(createSeedState("demo-tenant"));
  const service = new GovernanceService(store, new EventBus(store));
  const actor = { tenantId: "demo-tenant", userId: "test-user", role: "governanceos_admin" };
...
test("GovernanceOS overview uses seed data", () => {
...
test("GovernanceOS creates items, workflows, and runs", () => {
  const { service, actor, file } = createService();
  try {
...
test("GovernanceOS creates policies and audits changes", () => {
  const { service, actor, file } = createService();
  try {
