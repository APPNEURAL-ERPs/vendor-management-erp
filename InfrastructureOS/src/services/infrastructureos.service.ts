import { InfrastructureItem, InfrastructureOverview, InfrastructurePolicy, InfrastructureWorkflow, InfrastructureWorkRun, EntityStatus, Priority, RequestActor, WorkStatus } from "../core/domain";
...
export class InfrastructureService {
  ...
  listItems(actor: RequestActor, query?: URLSearchParams): InfrastructureItem[] {
  ...
  createItem(actor: RequestActor, input: Partial<InfrastructureItem>): InfrastructureItem {
    const state = this.store.getState();
    ...
    const item: InfrastructureItem = {
      ...
      category: ensureString(input.category ?? "cloud", "category"),
      ownerTeam: ensureString(input.ownerTeam ?? "Infrastructure", "ownerTeam"),
    ...
    this.store.save();
    this.store.audit(actor, "infrastructureos.item.create", "item", item.id, undefined, item);
    this.events.publish(actor, "infrastructureos.item.created", { itemKey: item.key });
    return clone(item);
  }

  updateItem(actor: RequestActor, key: string, input: Partial<InfrastructureItem>): InfrastructureItem {
    ...
    this.store.save();
    this.store.audit(actor, "infrastructureos.item.update", "item", item.id, before, item);
    this.events.publish(actor, "infrastructureos.item.updated", { itemKey: item.key });
    return clone(item);
  }

  archiveItem(actor: RequestActor, key: string): InfrastructureItem {
    return this.updateItem(actor, key, { status: "archived" });
  }

  listWorkflows(actor: RequestActor): InfrastructureWorkflow[] {
    return clone(this.store.getState().workflows.filter((item) => item.tenantId === actor.tenantId && item.status !== "archived"));
  }

  createWorkflow(actor: RequestActor, input: Partial<InfrastructureWorkflow>): InfrastructureWorkflow {
    const state = this.store.getState();
    const key = normalizeKey(input.key ?? input.name);
    this.ensureUnique(state.workflows, actor.tenantId, key, "Workflow");
    const now = nowIso();
    const workflow: InfrastructureWorkflow = {
      id: newId("flow"),
      tenantId: actor.tenantId,
      createdAt: now,
      updatedAt: now,
      ...
      description: optionalString(input.description),
      category: ensureString(input.category ?? "network", "category"),
      status: this.ensureStatus(input.status ?? "active"),
    ...
    this.store.save();
    this.store.audit(actor, "infrastructureos.workflow.create", "workflow", workflow.id, undefined, workflow);
    this.events.publish(actor, "infrastructureos.workflow.created", { workflowKey: workflow.key });
    return clone(workflow);
  }

  startWorkflow(actor: RequestActor, key: string, input: Record<string, unknown> = {}): InfrastructureWorkRun {
    const workflow = this.findWorkflow(actor, key);
    if (workflow.status !== "active") badRequest("Only active workflows can be started");
    const now = nowIso();
    const run: InfrastructureWorkRun = {
      id: newId("run"),
      tenantId: actor.tenantId,
      createdAt: now,
      updatedAt: now,
    ...
    this.store.save();
    this.store.audit(actor, "infrastructureos.run.create", "work_run", run.id, undefined, run);
    this.events.publish(actor, "infrastructureos.workflow.started", { runId: run.id, workflowKey: run.workflowKey });
    return clone(run);
  }

  updateRun(actor: RequestActor, id: string, input: Partial<InfrastructureWorkRun>): InfrastructureWorkRun {
    ...
    this.store.save();
    this.store.audit(actor, "infrastructureos.run.update", "work_run", run.id, before, run);
    this.events.publish(actor, "infrastructureos.run.updated", { runId: run.id, status: run.status });
    return clone(run);
  }

  listRuns(actor: RequestActor): InfrastructureWorkRun[] {
    return clone(this.store.getState().workRuns.filter((item) => item.tenantId === actor.tenantId));
  }

  createPolicy(actor: RequestActor, input: Partial<InfrastructurePolicy>): InfrastructurePolicy {
    const state = this.store.getState();
    const key = normalizeKey(input.key ?? input.title);
    this.ensureUnique(state.policies, actor.tenantId, key, "Policy");
    const now = nowIso();
    const policy: InfrastructurePolicy = {
      id: newId("pol"),
      tenantId: actor.tenantId,
      createdAt: now,
      updatedAt: now,
    ...
    this.store.save();
    this.store.audit(actor, "infrastructureos.policy.create", "policy", policy.id, undefined, policy);
    this.events.publish(actor, "infrastructureos.policy.created", { policyKey: policy.key });
    return clone(policy);
  }

  listPolicies(actor: RequestActor): InfrastructurePolicy[] {
  ...
  private findItem(actor: RequestActor, key: string): InfrastructureItem {
  ...
  private findWorkflow(actor: RequestActor, key: string): InfrastructureWorkflow {
  ...
  private findRun(actor: RequestActor, id: string): InfrastructureWorkRun {
