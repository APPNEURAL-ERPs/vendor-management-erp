...
const { InfrastructureService } = require("../dist/services/infrastructureos.service.js");
const { createSeedState } = require("../dist/seed-state.js");

function createService() {
  const file = join("/tmp", "infrastructureos-test-" + Date.now() + "-" + Math.random().toString(36).slice(2) + ".json");
  const store = new DataStore(file);
  store.reset(createSeedState("demo-tenant"));
  const service = new InfrastructureService(store, new EventBus(store));
  const actor = { tenantId: "demo-tenant", userId: "test-user", role: "infrastructureos_admin" };
...
test("InfrastructureOS overview uses seed data", () => {
...
test("InfrastructureOS creates items, workflows, and runs", () => {
  const { service, actor, file } = createService();
  try {
...
test("InfrastructureOS creates policies and audits changes", () => {
  const { service, actor, file } = createService();
  try {
