...
const { IntegrationService } = require("../dist/services/integrationos.service.js");
const { createSeedState } = require("../dist/seed-state.js");

function createService() {
  const file = join("/tmp", "integrationos-test-" + Date.now() + "-" + Math.random().toString(36).slice(2) + ".json");
  const store = new DataStore(file);
  store.reset(createSeedState("demo-tenant"));
  const service = new IntegrationService(store, new EventBus(store));
  const actor = { tenantId: "demo-tenant", userId: "test-user", role: "integrationos_admin" };
...
test("IntegrationOS overview uses seed data", () => {
...
test("IntegrationOS creates items, workflows, and runs", () => {
  const { service, actor, file } = createService();
  try {
...
test("IntegrationOS creates policies and audits changes", () => {
  const { service, actor, file } = createService();
  try {
