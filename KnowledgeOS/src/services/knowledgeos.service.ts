import { KnowledgeItem, KnowledgeOverview, KnowledgePolicy, KnowledgeWorkflow, KnowledgeWorkRun, EntityStatus, Priority, RequestActor, WorkStatus } from "../core/domain";
...
export class KnowledgeService {
  ...
  listItems(actor: RequestActor, query?: URLSearchParams): KnowledgeItem[] {
  ...
  createItem(actor: RequestActor, input: Partial<KnowledgeItem>): KnowledgeItem {
    const state = this.store.getState();
    ...
    const item: KnowledgeItem = {
      ...
      category: ensureString(input.category ?? "docs", "category"),
      ownerTeam: ensureString(input.ownerTeam ?? "Knowledge", "ownerTeam"),
    ...
    this.store.save();
    this.store.audit(actor, "knowledgeos.item.create", "item", item.id, undefined, item);
    this.events.publish(actor, "knowledgeos.item.created", { itemKey: item.key });
    return clone(item);
  }

  updateItem(actor: RequestActor, key: string, input: Partial<KnowledgeItem>): KnowledgeItem {
    ...
    this.store.save();
    this.store.audit(actor, "knowledgeos.item.update", "item", item.id, before, item);
    this.events.publish(actor, "knowledgeos.item.updated", { itemKey: item.key });
    return clone(item);
  }

  archiveItem(actor: RequestActor, key: string): KnowledgeItem {
    return this.updateItem(actor, key, { status: "archived" });
  }

  listWorkflows(actor: RequestActor): KnowledgeWorkflow[] {
    return clone(this.store.getState().workflows.filter((item) => item.tenantId === actor.tenantId && item.status !== "archived"));
  }

  createWorkflow(actor: RequestActor, input: Partial<KnowledgeWorkflow>): KnowledgeWorkflow {
    const state = this.store.getState();
    const key = normalizeKey(input.key ?? input.name);
    this.ensureUnique(state.workflows, actor.tenantId, key, "Workflow");
    const now = nowIso();
    const workflow: KnowledgeWorkflow = {
      id: newId("flow"),
      tenantId: actor.tenantId,
      createdAt: now,
      updatedAt: now,
      ...
      description: optionalString(input.description),
      category: ensureString(input.category ?? "wiki", "category"),
      status: this.ensureStatus(input.status ?? "active"),
    ...
    this.store.save();
    this.store.audit(actor, "knowledgeos.workflow.create", "workflow", workflow.id, undefined, workflow);
    this.events.publish(actor, "knowledgeos.workflow.created", { workflowKey: workflow.key });
    return clone(workflow);
  }

  startWorkflow(actor: RequestActor, key: string, input: Record<string, unknown> = {}): KnowledgeWorkRun {
    const workflow = this.findWorkflow(actor, key);
    if (workflow.status !== "active") badRequest("Only active workflows can be started");
    const now = nowIso();
    const run: KnowledgeWorkRun = {
      id: newId("run"),
      tenantId: actor.tenantId,
      createdAt: now,
      updatedAt: now,
    ...
    this.store.save();
    this.store.audit(actor, "knowledgeos.run.create", "work_run", run.id, undefined, run);
    this.events.publish(actor, "knowledgeos.workflow.started", { runId: run.id, workflowKey: run.workflowKey });
    return clone(run);
  }

  updateRun(actor: RequestActor, id: string, input: Partial<KnowledgeWorkRun>): KnowledgeWorkRun {
    ...
    this.store.save();
    this.store.audit(actor, "knowledgeos.run.update", "work_run", run.id, before, run);
    this.events.publish(actor, "knowledgeos.run.updated", { runId: run.id, status: run.status });
    return clone(run);
  }

  listRuns(actor: RequestActor): KnowledgeWorkRun[] {
    return clone(this.store.getState().workRuns.filter((item) => item.tenantId === actor.tenantId));
  }

  createPolicy(actor: RequestActor, input: Partial<KnowledgePolicy>): KnowledgePolicy {
    const state = this.store.getState();
    const key = normalizeKey(input.key ?? input.title);
    ...
    this.store.save();
    this.store.audit(actor, "knowledgeos.policy.create", "policy", policy.id, undefined, policy);
    this.events.publish(actor, "knowledgeos.policy.created", { policyKey: policy.key });
    return clone(policy);
  }

  listPolicies(actor: RequestActor): KnowledgePolicy[] {
  ...
  private findItem(actor: RequestActor, key: string): KnowledgeItem {
  ...
  private findWorkflow(actor: RequestActor, key: string): KnowledgeWorkflow {
  ...
  private findRun(actor: RequestActor, id: string): KnowledgeWorkRun {
