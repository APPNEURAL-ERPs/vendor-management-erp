...
const { LegalService } = require("../dist/services/legalos.service.js");
const { createSeedState } = require("../dist/seed-state.js");

function createService() {
  const file = join("/tmp", "legalos-test-" + Date.now() + "-" + Math.random().toString(36).slice(2) + ".json");
  const store = new DataStore(file);
  store.reset(createSeedState("demo-tenant"));
  const service = new LegalService(store, new EventBus(store));
  const actor = { tenantId: "demo-tenant", userId: "test-user", role: "legalos_admin" };
...
test("LegalOS overview uses seed data", () => {
...
test("LegalOS creates items, workflows, and runs", () => {
  const { service, actor, file } = createService();
  try {
...
test("LegalOS creates policies and audits changes", () => {
  const { service, actor, file } = createService();
  try {
