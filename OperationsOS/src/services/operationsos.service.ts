import { OperationsItem, OperationsOverview, OperationsPolicy, OperationsWorkflow, OperationsWorkRun, EntityStatus, Priority, RequestActor, WorkStatus } from "../core/domain";
...
export class OperationsService {
  ...
  listItems(actor: RequestActor, query?: URLSearchParams): OperationsItem[] {
  ...
  createItem(actor: RequestActor, input: Partial<OperationsItem>): OperationsItem {
    const state = this.store.getState();
    ...
    const item: OperationsItem = {
      ...
      category: ensureString(input.category ?? "sop", "category"),
      ownerTeam: ensureString(input.ownerTeam ?? "Operations", "ownerTeam"),
    ...
    this.store.save();
    this.store.audit(actor, "operationsos.item.create", "item", item.id, undefined, item);
    this.events.publish(actor, "operationsos.item.created", { itemKey: item.key });
    return clone(item);
  }

  updateItem(actor: RequestActor, key: string, input: Partial<OperationsItem>): OperationsItem {
    ...
    this.store.save();
    this.store.audit(actor, "operationsos.item.update", "item", item.id, before, item);
    this.events.publish(actor, "operationsos.item.updated", { itemKey: item.key });
    return clone(item);
  }

  archiveItem(actor: RequestActor, key: string): OperationsItem {
    return this.updateItem(actor, key, { status: "archived" });
  }

  listWorkflows(actor: RequestActor): OperationsWorkflow[] {
    return clone(this.store.getState().workflows.filter((item) => item.tenantId === actor.tenantId && item.status !== "archived"));
  }

  createWorkflow(actor: RequestActor, input: Partial<OperationsWorkflow>): OperationsWorkflow {
    const state = this.store.getState();
    const key = normalizeKey(input.key ?? input.name);
    this.ensureUnique(state.workflows, actor.tenantId, key, "Workflow");
    const now = nowIso();
    const workflow: OperationsWorkflow = {
      id: newId("flow"),
      tenantId: actor.tenantId,
      createdAt: now,
      updatedAt: now,
      ...
      description: optionalString(input.description),
      category: ensureString(input.category ?? "task", "category"),
      status: this.ensureStatus(input.status ?? "active"),
    ...
    this.store.save();
    this.store.audit(actor, "operationsos.workflow.create", "workflow", workflow.id, undefined, workflow);
    this.events.publish(actor, "operationsos.workflow.created", { workflowKey: workflow.key });
    return clone(workflow);
  }

  startWorkflow(actor: RequestActor, key: string, input: Record<string, unknown> = {}): OperationsWorkRun {
    const workflow = this.findWorkflow(actor, key);
    if (workflow.status !== "active") badRequest("Only active workflows can be started");
    const now = nowIso();
    const run: OperationsWorkRun = {
      id: newId("run"),
      tenantId: actor.tenantId,
      createdAt: now,
      updatedAt: now,
    ...
    this.store.save();
    this.store.audit(actor, "operationsos.run.create", "work_run", run.id, undefined, run);
    this.events.publish(actor, "operationsos.workflow.started", { runId: run.id, workflowKey: run.workflowKey });
    return clone(run);
  }

  updateRun(actor: RequestActor, id: string, input: Partial<OperationsWorkRun>): OperationsWorkRun {
    ...
    this.store.save();
    this.store.audit(actor, "operationsos.run.update", "work_run", run.id, before, run);
    this.events.publish(actor, "operationsos.run.updated", { runId: run.id, status: run.status });
    return clone(run);
  }

  listRuns(actor: RequestActor): OperationsWorkRun[] {
    return clone(this.store.getState().workRuns.filter((item) => item.tenantId === actor.tenantId));
  }

  createPolicy(actor: RequestActor, input: Partial<OperationsPolicy>): OperationsPolicy {
    const state = this.store.getState();
    const key = normalizeKey(input.key ?? input.title);
    ...
    this.store.save();
    this.store.audit(actor, "operationsos.policy.create", "policy", policy.id, undefined, policy);
    this.events.publish(actor, "operationsos.policy.created", { policyKey: policy.key });
    return clone(policy);
  }

  listPolicies(actor: RequestActor): OperationsPolicy[] {
  ...
  private findItem(actor: RequestActor, key: string): OperationsItem {
  ...
  private findWorkflow(actor: RequestActor, key: string): OperationsWorkflow {
  ...
  private findRun(actor: RequestActor, id: string): OperationsWorkRun {
