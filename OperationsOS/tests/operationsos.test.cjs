...
const { OperationsService } = require("../dist/services/operationsos.service.js");
const { createSeedState } = require("../dist/seed-state.js");

function createService() {
  const file = join("/tmp", "operationsos-test-" + Date.now() + "-" + Math.random().toString(36).slice(2) + ".json");
  const store = new DataStore(file);
  store.reset(createSeedState("demo-tenant"));
  const service = new OperationsService(store, new EventBus(store));
  const actor = { tenantId: "demo-tenant", userId: "test-user", role: "operationsos_admin" };
...
test("OperationsOS overview uses seed data", () => {
...
test("OperationsOS creates items, workflows, and runs", () => {
  const { service, actor, file } = createService();
  try {
...
test("OperationsOS creates policies and audits changes", () => {
  const { service, actor, file } = createService();
  try {
