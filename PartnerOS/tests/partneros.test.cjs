...
const { PartnerService } = require("../dist/services/partneros.service.js");
const { createSeedState } = require("../dist/seed-state.js");

function createService() {
  const file = join("/tmp", "partneros-test-" + Date.now() + "-" + Math.random().toString(36).slice(2) + ".json");
  const store = new DataStore(file);
  store.reset(createSeedState("demo-tenant"));
  const service = new PartnerService(store, new EventBus(store));
  const actor = { tenantId: "demo-tenant", userId: "test-user", role: "partneros_admin" };
...
test("PartnerOS overview uses seed data", () => {
...
test("PartnerOS creates items, workflows, and runs", () => {
  const { service, actor, file } = createService();
  try {
...
test("PartnerOS creates policies and audits changes", () => {
  const { service, actor, file } = createService();
  try {
