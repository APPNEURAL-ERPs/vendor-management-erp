import { QualityItem, QualityOverview, QualityPolicy, QualityWorkflow, QualityWorkRun, EntityStatus, Priority, RequestActor, WorkStatus } from "../core/domain";
...
export class QualityService {
  ...
  listItems(actor: RequestActor, query?: URLSearchParams): QualityItem[] {
  ...
  createItem(actor: RequestActor, input: Partial<QualityItem>): QualityItem {
    const state = this.store.getState();
    ...
    const item: QualityItem = {
      ...
      category: ensureString(input.category ?? "qa", "category"),
      ownerTeam: ensureString(input.ownerTeam ?? "Quality", "ownerTeam"),
    ...
    this.store.save();
    this.store.audit(actor, "qualityos.item.create", "item", item.id, undefined, item);
    this.events.publish(actor, "qualityos.item.created", { itemKey: item.key });
    return clone(item);
  }

  updateItem(actor: RequestActor, key: string, input: Partial<QualityItem>): QualityItem {
    ...
    this.store.save();
    this.store.audit(actor, "qualityos.item.update", "item", item.id, before, item);
    this.events.publish(actor, "qualityos.item.updated", { itemKey: item.key });
    return clone(item);
  }

  archiveItem(actor: RequestActor, key: string): QualityItem {
    return this.updateItem(actor, key, { status: "archived" });
  }

  listWorkflows(actor: RequestActor): QualityWorkflow[] {
    return clone(this.store.getState().workflows.filter((item) => item.tenantId === actor.tenantId && item.status !== "archived"));
  }

  createWorkflow(actor: RequestActor, input: Partial<QualityWorkflow>): QualityWorkflow {
    const state = this.store.getState();
    ...
    const workflow: QualityWorkflow = {
      ...
      description: optionalString(input.description),
      category: ensureString(input.category ?? "testing", "category"),
      status: this.ensureStatus(input.status ?? "active"),
    ...
    this.store.save();
    this.store.audit(actor, "qualityos.workflow.create", "workflow", workflow.id, undefined, workflow);
    this.events.publish(actor, "qualityos.workflow.created", { workflowKey: workflow.key });
    return clone(workflow);
  }

  startWorkflow(actor: RequestActor, key: string, input: Record<string, unknown> = {}): QualityWorkRun {
    const workflow = this.findWorkflow(actor, key);
    if (workflow.status !== "active") badRequest("Only active workflows can be started");
    const now = nowIso();
    const run: QualityWorkRun = {
      id: newId("run"),
      tenantId: actor.tenantId,
      createdAt: now,
      updatedAt: now,
    ...
    this.store.save();
    this.store.audit(actor, "qualityos.run.create", "work_run", run.id, undefined, run);
    this.events.publish(actor, "qualityos.workflow.started", { runId: run.id, workflowKey: run.workflowKey });
    return clone(run);
  }

  updateRun(actor: RequestActor, id: string, input: Partial<QualityWorkRun>): QualityWorkRun {
    ...
    this.store.save();
    this.store.audit(actor, "qualityos.run.update", "work_run", run.id, before, run);
    this.events.publish(actor, "qualityos.run.updated", { runId: run.id, status: run.status });
    return clone(run);
  }

  listRuns(actor: RequestActor): QualityWorkRun[] {
    return clone(this.store.getState().workRuns.filter((item) => item.tenantId === actor.tenantId));
  }

  createPolicy(actor: RequestActor, input: Partial<QualityPolicy>): QualityPolicy {
    const state = this.store.getState();
    const key = normalizeKey(input.key ?? input.title);
    ...
    this.store.save();
    this.store.audit(actor, "qualityos.policy.create", "policy", policy.id, undefined, policy);
    this.events.publish(actor, "qualityos.policy.created", { policyKey: policy.key });
    return clone(policy);
  }

  listPolicies(actor: RequestActor): QualityPolicy[] {
  ...
  private findItem(actor: RequestActor, key: string): QualityItem {
  ...
  private findWorkflow(actor: RequestActor, key: string): QualityWorkflow {
  ...
  private findRun(actor: RequestActor, id: string): QualityWorkRun {
