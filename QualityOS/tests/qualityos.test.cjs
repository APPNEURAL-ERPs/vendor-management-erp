...
const { QualityService } = require("../dist/services/qualityos.service.js");
const { createSeedState } = require("../dist/seed-state.js");

function createService() {
  const file = join("/tmp", "qualityos-test-" + Date.now() + "-" + Math.random().toString(36).slice(2) + ".json");
  const store = new DataStore(file);
  store.reset(createSeedState("demo-tenant"));
  const service = new QualityService(store, new EventBus(store));
  const actor = { tenantId: "demo-tenant", userId: "test-user", role: "qualityos_admin" };
...
test("QualityOS overview uses seed data", () => {
...
test("QualityOS creates items, workflows, and runs", () => {
  const { service, actor, file } = createService();
  try {
...
test("QualityOS creates policies and audits changes", () => {
  const { service, actor, file } = createService();
  try {
