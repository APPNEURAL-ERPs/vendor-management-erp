...
export type Role = "owner" | "admin" | "researchos_admin" | "manager" | "operator" | "auditor" | "viewer";
...
export interface ResearchWorkflow extends BaseEntity {
  key: string;
  name: string;
  description?: string;
  category: string;
...
export interface ResearchWorkRun extends BaseEntity {
  workflowKey: string;
  status: WorkStatus;
  requestedBy: UUID;
  currentStep?: string;
...
export interface ResearchPolicy extends BaseEntity {
  key: string;
  title: string;
  body: string;
  status: EntityStatus;
