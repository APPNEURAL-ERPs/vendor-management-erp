import { ResearchItem, ResearchOverview, ResearchPolicy, ResearchWorkflow, ResearchWorkRun, EntityStatus, Priority, RequestActor, WorkStatus } from "../core/domain";
...
export class ResearchService {
  ...
  overview(actor: RequestActor): ResearchOverview {
  ...
  listItems(actor: RequestActor, query?: URLSearchParams): ResearchItem[] {
  ...
  createItem(actor: RequestActor, input: Partial<ResearchItem>): ResearchItem {
    const state = this.store.getState();
    ...
    const item: ResearchItem = {
      ...
      category: ensureString(input.category ?? "market", "category"),
      ownerTeam: ensureString(input.ownerTeam ?? "Research", "ownerTeam"),
    ...
    this.store.save();
    this.store.audit(actor, "researchos.item.create", "item", item.id, undefined, item);
    this.events.publish(actor, "researchos.item.created", { itemKey: item.key });
    return clone(item);
  }

  updateItem(actor: RequestActor, key: string, input: Partial<ResearchItem>): ResearchItem {
    ...
    this.store.save();
    this.store.audit(actor, "researchos.item.update", "item", item.id, before, item);
    this.events.publish(actor, "researchos.item.updated", { itemKey: item.key });
    return clone(item);
  }

  archiveItem(actor: RequestActor, key: string): ResearchItem {
    return this.updateItem(actor, key, { status: "archived" });
  }

  listWorkflows(actor: RequestActor): ResearchWorkflow[] {
    return clone(this.store.getState().workflows.filter((item) => item.tenantId === actor.tenantId && item.status !== "archived"));
  }

  createWorkflow(actor: RequestActor, input: Partial<ResearchWorkflow>): ResearchWorkflow {
    const state = this.store.getState();
    ...
    const workflow: ResearchWorkflow = {
      ...
      description: optionalString(input.description),
      category: ensureString(input.category ?? "user", "category"),
      status: this.ensureStatus(input.status ?? "active"),
    ...
    this.store.save();
    this.store.audit(actor, "researchos.workflow.create", "workflow", workflow.id, undefined, workflow);
    this.events.publish(actor, "researchos.workflow.created", { workflowKey: workflow.key });
    return clone(workflow);
  }

  startWorkflow(actor: RequestActor, key: string, input: Record<string, unknown> = {}): ResearchWorkRun {
    const workflow = this.findWorkflow(actor, key);
    if (workflow.status !== "active") badRequest("Only active workflows can be started");
    const now = nowIso();
    const run: ResearchWorkRun = {
      id: newId("run"),
      tenantId: actor.tenantId,
      createdAt: now,
      updatedAt: now,
    ...
    this.store.save();
    this.store.audit(actor, "researchos.run.create", "work_run", run.id, undefined, run);
    this.events.publish(actor, "researchos.workflow.started", { runId: run.id, workflowKey: run.workflowKey });
    return clone(run);
  }

  updateRun(actor: RequestActor, id: string, input: Partial<ResearchWorkRun>): ResearchWorkRun {
    ...
    this.store.save();
    this.store.audit(actor, "researchos.run.update", "work_run", run.id, before, run);
    this.events.publish(actor, "researchos.run.updated", { runId: run.id, status: run.status });
    return clone(run);
  }

  listRuns(actor: RequestActor): ResearchWorkRun[] {
    return clone(this.store.getState().workRuns.filter((item) => item.tenantId === actor.tenantId));
  }

  createPolicy(actor: RequestActor, input: Partial<ResearchPolicy>): ResearchPolicy {
    const state = this.store.getState();
    const key = normalizeKey(input.key ?? input.title);
    ...
    this.store.save();
    this.store.audit(actor, "researchos.policy.create", "policy", policy.id, undefined, policy);
    this.events.publish(actor, "researchos.policy.created", { policyKey: policy.key });
    return clone(policy);
  }

  listPolicies(actor: RequestActor): ResearchPolicy[] {
  ...
  private findItem(actor: RequestActor, key: string): ResearchItem {
  ...
  private findWorkflow(actor: RequestActor, key: string): ResearchWorkflow {
  ...
  private findRun(actor: RequestActor, id: string): ResearchWorkRun {
