...
const { ResearchService } = require("../dist/services/researchos.service.js");
const { createSeedState } = require("../dist/seed-state.js");

function createService() {
  const file = join("/tmp", "researchos-test-" + Date.now() + "-" + Math.random().toString(36).slice(2) + ".json");
  const store = new DataStore(file);
  store.reset(createSeedState("demo-tenant"));
  const service = new ResearchService(store, new EventBus(store));
  const actor = { tenantId: "demo-tenant", userId: "test-user", role: "researchos_admin" };
...
test("ResearchOS overview uses seed data", () => {
...
test("ResearchOS creates items, workflows, and runs", () => {
  const { service, actor, file } = createService();
  try {
...
test("ResearchOS creates policies and audits changes", () => {
  const { service, actor, file } = createService();
  try {
