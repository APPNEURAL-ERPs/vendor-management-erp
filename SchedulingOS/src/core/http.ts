...
const validRoles: Role[] = ["owner", "admin", "scheduler", "scheduler_manager", "viewer"];
...
function hasPermission(role: Role, permission?: string): boolean {
  ...
  if (role === "scheduler" && permission.startsWith("scheduling.")) return true;
  if (role === "scheduler_manager" && permission.startsWith("scheduling.")) return true;
  if (role === "viewer" && permission.includes(".read")) return true;
  return false;
}

function requirePermission(role: Role, permission?: string): void {
  if (!hasPermission(role, permission)) {
    throw new HttpError(403, `Role ${role} does not have permission ${permission ?? "access"}`);
  }
}
