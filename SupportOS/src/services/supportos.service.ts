import { SupportItem, SupportOverview, SupportPolicy, SupportWorkflow, SupportWorkRun, EntityStatus, Priority, RequestActor, WorkStatus } from "../core/domain";
...
export class SupportService {
  ...
  listItems(actor: RequestActor, query?: URLSearchParams): SupportItem[] {
  ...
  createItem(actor: RequestActor, input: Partial<SupportItem>): SupportItem {
    const state = this.store.getState();
    ...
    const item: SupportItem = {
      ...
      category: ensureString(input.category ?? "ticket", "category"),
      ownerTeam: ensureString(input.ownerTeam ?? "Support", "ownerTeam"),
    ...
    this.store.save();
    this.store.audit(actor, "supportos.item.create", "item", item.id, undefined, item);
    this.events.publish(actor, "supportos.item.created", { itemKey: item.key });
    return clone(item);
  }

  updateItem(actor: RequestActor, key: string, input: Partial<SupportItem>): SupportItem {
    ...
    this.store.save();
    this.store.audit(actor, "supportos.item.update", "item", item.id, before, item);
    this.events.publish(actor, "supportos.item.updated", { itemKey: item.key });
    return clone(item);
  }

  archiveItem(actor: RequestActor, key: string): SupportItem {
    return this.updateItem(actor, key, { status: "archived" });
  }

  listWorkflows(actor: RequestActor): SupportWorkflow[] {
    return clone(this.store.getState().workflows.filter((item) => item.tenantId === actor.tenantId && item.status !== "archived"));
  }

  createWorkflow(actor: RequestActor, input: Partial<SupportWorkflow>): SupportWorkflow {
    const state = this.store.getState();
    ...
    const workflow: SupportWorkflow = {
      ...
      description: optionalString(input.description),
      category: ensureString(input.category ?? "helpdesk", "category"),
      status: this.ensureStatus(input.status ?? "active"),
    ...
    this.store.save();
    this.store.audit(actor, "supportos.workflow.create", "workflow", workflow.id, undefined, workflow);
    this.events.publish(actor, "supportos.workflow.created", { workflowKey: workflow.key });
    return clone(workflow);
  }

  startWorkflow(actor: RequestActor, key: string, input: Record<string, unknown> = {}): SupportWorkRun {
    const workflow = this.findWorkflow(actor, key);
    if (workflow.status !== "active") badRequest("Only active workflows can be started");
    const now = nowIso();
    const run: SupportWorkRun = {
      id: newId("run"),
      tenantId: actor.tenantId,
      createdAt: now,
      updatedAt: now,
    ...
    this.store.save();
    this.store.audit(actor, "supportos.run.create", "work_run", run.id, undefined, run);
    this.events.publish(actor, "supportos.workflow.started", { runId: run.id, workflowKey: run.workflowKey });
    return clone(run);
  }

  updateRun(actor: RequestActor, id: string, input: Partial<SupportWorkRun>): SupportWorkRun {
    ...
    this.store.save();
    this.store.audit(actor, "supportos.run.update", "work_run", run.id, before, run);
    this.events.publish(actor, "supportos.run.updated", { runId: run.id, status: run.status });
    return clone(run);
  }

  listRuns(actor: RequestActor): SupportWorkRun[] {
    return clone(this.store.getState().workRuns.filter((item) => item.tenantId === actor.tenantId));
  }

  createPolicy(actor: RequestActor, input: Partial<SupportPolicy>): SupportPolicy {
    const state = this.store.getState();
    const key = normalizeKey(input.key ?? input.title);
    ...
    this.store.save();
    this.store.audit(actor, "supportos.policy.create", "policy", policy.id, undefined, policy);
    this.events.publish(actor, "supportos.policy.created", { policyKey: policy.key });
    return clone(policy);
  }

  listPolicies(actor: RequestActor): SupportPolicy[] {
  ...
  private findItem(actor: RequestActor, key: string): SupportItem {
  ...
  private findWorkflow(actor: RequestActor, key: string): SupportWorkflow {
  ...
  private findRun(actor: RequestActor, id: string): SupportWorkRun {
