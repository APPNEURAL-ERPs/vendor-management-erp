...
const { SupportService } = require("../dist/services/supportos.service.js");
const { createSeedState } = require("../dist/seed-state.js");

function createService() {
  const file = join("/tmp", "supportos-test-" + Date.now() + "-" + Math.random().toString(36).slice(2) + ".json");
  const store = new DataStore(file);
  store.reset(createSeedState("demo-tenant"));
  const service = new SupportService(store, new EventBus(store));
  const actor = { tenantId: "demo-tenant", userId: "test-user", role: "supportos_admin" };
...
test("SupportOS overview uses seed data", () => {
...
test("SupportOS creates items, workflows, and runs", () => {
  const { service, actor, file } = createService();
  try {
...
test("SupportOS creates policies and audits changes", () => {
  const { service, actor, file } = createService();
  try {
