...
import { UnifiedService } from "./service";
import { createSeedState } from "./seed-state";
import { docs } from "./docs";
import { RequestActor, Role } from "./domain";

const roles: Role[] = ["owner", "admin", "unified_admin", "unified_operator", "viewer"];
const port = Number(process.env.PORT ?? 10717);
const dbFile = process.env.UNIFIEDOS_DB_FILE ?? "data/unifiedos.db.json";
...
const service = new UnifiedService(store);
...
const server = createServer(async (req, res) => {
  try {
    ...
    if (req.method === "GET" && url.pathname === "/health") { sendJson(res, 200, { service: "UnifiedOS", status: "ok", message: service.getRoutesSummary() }); return; }
    if (req.method === "GET" && url.pathname === "/docs") { sendJson(res, 200, docs()); return; }
    if (parts[0] !== "unifiedos") notFound("Route not found");
...
server.listen(port, () => { console.log("UnifiedOS listening on http://localhost:" + port); });
